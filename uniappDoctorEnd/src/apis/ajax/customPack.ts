...
const specialHandl = (url: string, params: IOBJ, options: IMyOptions) => {
    ...
    if (url === '/his/request') {
        const {api, ...o} = params;
        let {synUserName = '', synKey = '', UserName = '', organdoctorId = ''}: IOBJ = {};
        options.noCloseBeforeAjax = true;

        // debugger;
        if (store && store.getters.isLogin) {
            const user = store.getters.userInfo;
            const temporaryCode = store.getters.temporaryCode;

            synUserName = user.userName || '';
            synKey = user.synKey || '';
            UserName = user.userName || '';
            organdoctorId = temporaryCode || user.UserCode || '';
        }
